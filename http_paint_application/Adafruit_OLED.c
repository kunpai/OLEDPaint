...
unsigned int storedcolor[128][128]; // bitmap to store color at every pixel
...
unsigned long readData(int x, int y){
  goTo(x,y);
  unsigned long get;
  MAP_SPICSEnable(GSPI_BASE);
  GPIOPinWrite(GPIOA3_BASE, 0x10, 0x00); // OC set as low
...
void drawFastVLine(int x, int y, int h, unsigned int color) {
  ...
  for(j = y; j<= (y+h-1); j++){
      storedcolor[x][j] = color;
  }
}

void drawFastVLineCursor(int x, int y, int h) {
  ...
  for(j = y; j<= (y+h-1); j++){
      unsigned int color = storedcolor[x][j];
      writeData(color >> 8);
      writeData(color);
  }
...
void drawPixel(int x, int y, unsigned int color)
{
  ...
  storedcolor[x][y] = color;
}

void drawCursorPixel(int x, int y, unsigned int color)
{
  if ((x >= SSD1351WIDTH) || (y >= SSD1351HEIGHT)) return;
  if ((x < 0) || (y < 0)) return;
